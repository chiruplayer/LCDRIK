...
from lasagne.nonlinearities import rectify, softmax, very_leaky_rectify, sigmoid, LeakyRectify
...
#print(newfiles)
'''
Lasagne Model yolo and Batch Iterator
'''
def yolo(input_var=None):
    l_in = InputLayer(shape=(None, 1, PIXELS, PIXELS), input_var=input_var)

    l_conv = ConvLayer(l_in, num_filters=16, filter_size=3, pad=1, nonlinearity=very_leaky_rectify)
    ...
    l_convb = ConvLayer(l_pool, num_filters=32, filter_size=3, pad=1, nonlinearity=very_leaky_rectify)
    
    l_poolb = MaxPool2DLayer(l_convb , pool_size=2, stride = 2) 
    
    l_convc = ConvLayer(l_poolb, num_filters=64, filter_size=3, pad=1, nonlinearity=very_leaky_rectify)
    
    l_poolc = MaxPool2DLayer(l_convc , pool_size=2, stride = 2)

    l_convd = ConvLayer(l_poolc, num_filters=128, filter_size=3, pad=1, nonlinearity=very_leaky_rectify)
    #l_dropout1 = DropoutLayer(l_pool, p=0.25)
    
    l_poold = MaxPool2DLayer(l_convd , pool_size=2, stride = 2)
    
    l_conve = ConvLayer(l_poold, num_filters=256, filter_size=3, pad=1, nonlinearity=very_leaky_rectify)

    l_poole = MaxPool2DLayer(l_conve , pool_size=2, stride = 2)
    
    l_convf = ConvLayer(l_poole, num_filters=512, filter_size=3, pad=1, nonlinearity=very_leaky_rectify)
    
    l_poolf = MaxPool2DLayer(l_convf , pool_size=2, stride = 2)

    l_hidden = DenseLayer(l_pool, num_units=1024, nonlinearity=very_leaky_rectify)
    #l_dropout2 = DropoutLayer(l_hidden, p=0.5)
        
    l_hidden1 = DenseLayer(l_hidden, num_units=512, nonlinearity=very_leaky_rectify)

    l_hidden2 = DenseLayer(l_hidden1, num_units=256, nonlinearity=very_leaky_rectify)

    l_hidden3 = DenseLayer(l_hidden2, num_units=128, nonlinearity=very_leaky_rectify)

    l_hidden4 = DenseLayer(l_hidden2, num_units=64, nonlinearity=very_leaky_rectify)
...
def create_submission(predictions, test_id):
    #pass
    result1 = pd.DataFrame(predictions, columns=['c0', 'c1'])
